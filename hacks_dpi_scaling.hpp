...
namespace toolbox::hacks_dpi_scaling
{
     ...
     * recommended : DPI scaling value reommended by OS. OS takes resolution, physical size, and expected viewing
     * distance into account while calculating this, however exact formula is not known, hence must be retrieved from OS
     *               For a system in which user has not explicitly changed DPI, current should eqaul recommended.
     * init_done :   If true, it means that the members of the struct contain values, as fetched from OS, and not the
     * default ones given while object creation.
     */
    struct DPIScalingInfo
    {
        UINT32 mininum = 100;
