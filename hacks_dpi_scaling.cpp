...
bool hacks_dpi_scaling::get_paths_and_modes(std::vector<DISPLAYCONFIG_PATH_INFO>& paths, std::vector<DISPLAYCONFIG_MODE_INFO>& modes, int flags)
{
...
hacks_dpi_scaling::DPIScalingInfo hacks_dpi_scaling::get_dpi_scaling_info(LUID adapter_id, UINT32 source_id)
{
    DPIScalingInfo dpi_info = {};

    hacks_dpi_scaling::DISPLAYCONFIG_SOURCE_DPI_SCALE_GET request_packet = {};
    request_packet.header.type = (DISPLAYCONFIG_DEVICE_INFO_TYPE)hacks_dpi_scaling::DISPLAYCONFIG_DEVICE_INFO_TYPE_CUSTOM::DISPLAYCONFIG_DEVICE_INFO_GET_DPI_SCALE;
    ...
    {//success
        ...
        if (hacks_dpi_scaling::count_of(dpi_vals) >= (size_t)(min_abs + request_packet.max_scale_rel + 1))
        {//all ok
...
bool hacks_dpi_scaling::set_dpi_scaling(LUID adapter_id, UINT32 source_id, UINT32 dpi_percent_to_set)
{
    ...
    hacks_dpi_scaling::DISPLAYCONFIG_SOURCE_DPI_SCALE_SET set_packet = {};
    ...
    set_packet.header.type = (DISPLAYCONFIG_DEVICE_INFO_TYPE) hacks_dpi_scaling::DISPLAYCONFIG_DEVICE_INFO_TYPE_CUSTOM::DISPLAYCONFIG_DEVICE_INFO_SET_DPI_SCALE;
...
*/
int hacks_dpi_scaling::get_recommended_dpi_scaling()
{
    int dpi = 0;
...
std::vector<hacks_dpi_scaling::DisplayData> hacks_dpi_scaling::get_display_data()
{
    std::vector<DisplayData> display_data_cache;
    std::vector<DISPLAYCONFIG_PATH_INFO> paths;
    std::vector<DISPLAYCONFIG_MODE_INFO> modes;
    int flags = QDC_ONLY_ACTIVE_PATHS;
    if (false == hacks_dpi_scaling::get_paths_and_modes(paths, modes, flags))
    {
...
bool hacks_dpi_scaling::dpi_found(int val)
{
    bool found = false;
    for (int i = 0; i < 12; i++)
    {
